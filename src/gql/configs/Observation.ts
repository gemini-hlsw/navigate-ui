...
const MUTATE_OBSERVATION = gql`
  mutation updateObservation(
    ...
    updateObservation(
      id: $id
      name: $name
      selectedGuideTarget: $selectedGuideTarget
...
const UPDATE_OBSERVATION_SELECTED_PROBE = gql`
  mutation updateObservationSelectedProbe($pk: Int, $selectedProbe: Int) {
...
const UPDATE_OBSERVATION_SELECTED_GUIDE_TARGET = gql`
  mutation updateObservationSelectedProbe(
    $pk: Int
    $selectedGuideTarget: String
  ) {
...
export function useUpdateObservationSelectedGuideTarget() {
  const [mutationFunction, { data, loading, error }] = useMutation(
    UPDATE_OBSERVATION_SELECTED_GUIDE_TARGET,
    {
      context: { clientName: "navigateConfigs" },
    }
  )
