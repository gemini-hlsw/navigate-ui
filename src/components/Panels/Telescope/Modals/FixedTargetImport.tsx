...
import { useGetFixedTargets } from "../../../../gql/??????"
import { AuthContext } from "../../../Contexts/Auth/AuthProvider"
import { TelescopeContext } from "../TelescopeProvider"

export function FixedTargetImport() {
  const { canEdit } = useContext(AuthContext)
  const { targetVisible, setTargetVisible, setFixedTarget } =
    useContext(TelescopeContext)
  ...
  return (
    <Dialog
      header="Import from Fixed Targets"
      footer={footer}
      visible={targetVisible}
      style={{ width: "50vw" }}
      modal
      onHide={() => setTargetVisible(false)}
    >
      {!Boolean(data) ? (
