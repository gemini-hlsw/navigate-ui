    ...
    <div className="systems">
      ...
      <div className={`grid-wrapper ${(collapsed) ? "collapsed" : ""}`}>
        ...
        <div className="bottom">
          ...
          <Dropdown disabled={!canEdit} style={{ gridArea: "g33" }} value={domeMode} options={DOME_MODE} onChange={(e) => setDomeMode(e.value)} placeholder="Select a Dome Mode" />
          <Button disabled={!canEdit} style={{ gridArea: "g36" }} label="Set" />
          <Button disabled={!canEdit} style={{ gridArea: "g41" }} label="Park" />
          <span style={{ textAlign: "center", alignSelf: "center", gridArea: "g42" }}>Mode</span>
          <Dropdown disabled={!canEdit} style={{ gridArea: "g43" }} value={shuttersMode} options={SHUTTERS_MODE} onChange={(e) => setShuttersMode(e.value)} placeholder="Select a Shutters Mode" />
          <span style={{ textAlign: "center", alignSelf: "center", gridArea: "g44" }}>Aperture</span>
          <InputNumber disabled={!canEdit} style={{ gridArea: "g45" }} value={aperture} onValueChange={(e) => setAperture((e.value) ? e.value : 0)} mode="decimal" />
          <Button disabled={!canEdit} style={{ gridArea: "g46" }} label="Set" />
          <Button disabled={!canEdit} style={{ gridArea: "g51" }} label="Close" />
          <InputNumber disabled={!canEdit} style={{ gridArea: "g52" }} value={WVGate} onValueChange={(e) => setWVGate((e.value) ? e.value : 0)} mode="decimal" />
          <Slider disabled={!canEdit} style={{ gridArea: "g53", marginTop: "10px" }} value={WVGate} onChange={(e) => setWVGate(e.value)} />
          <Button disabled={!canEdit} style={{ gridArea: "g56" }} label="Move" />
          <Button disabled={!canEdit} style={{ gridArea: "g61" }} label="Close" />
          <InputNumber disabled={!canEdit} style={{ gridArea: "g62" }} value={EVGate} onValueChange={(e) => setEVGate((e.value) ? e.value : 0)} mode="decimal" />
          <Slider disabled={!canEdit} style={{ gridArea: "g63", marginTop: "10px" }} value={EVGate} onChange={(e) => setEVGate(e.value)} />
